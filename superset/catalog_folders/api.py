...
class CatalogFolderRestApi(BaseSupersetModelRestApi):
    ...
    add_columns = ["name", "parent_id", "description", "color", "position"]
    ...
    def post(self) -> Response:
        ...
        try:
            ...
            db.session.commit()
        except CatalogFolderInvalidParentError as ex:
            db.session.rollback()
    ...
    def delete(self, pk: int) -> Response:  # type: ignore[override]
        """Удалить папку каталога. Подпапки становятся корневыми."""
        try:
            CatalogFolderDAO.delete(pk)
            db.session.commit()
        except CatalogFolderNotFoundError:
            db.session.rollback()
            return self.response_404()
    ...
    def tree(self) -> Response:
        ...
        return self.response(200, result=CatalogFolderDAO.get_tree())
