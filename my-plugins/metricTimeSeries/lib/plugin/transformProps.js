...
function findTimeKey(row) {
    ...
    for (const key of Object.keys(row)) {
        if ((0, dateHelpers_1.parseTimeValue)(row[key]))
            return key;
    }
    return null;
}
...
function toTimePoint(row, timeKey, factKey, planKey, pyKey) {
    const d = (0, dateHelpers_1.parseTimeValue)(row[timeKey]);
    if (!d)
        return null;
    ...
    return {
        ...
        year,
        month,
        monthName: (0, dateHelpers_1.ruMonthName)(month),
        day,
        week: (0, dateHelpers_1.isoWeekNumber)(d),
...
function buildCategorySeries(categoryRows, timePoints, groupbyColumn, factKey, limit) {
    ...
    const result = top.map(([name, values], i) => {
        const entry = (0, colorPalette_1.pickPaletteEntry)(i);
        return {
            id: name,
            name,
    ...
    if (tail.length > 0) {
        ...
        result.push({
            id: '__other__',
            name: 'Прочее',
            colorLight: colorPalette_1.OTHER_COLOR.colorLight,
            colorDark: colorPalette_1.OTHER_COLOR.colorDark,
            colorToken: colorPalette_1.OTHER_COLOR.colorToken,
            values: merged,
            total: tailTotal,
        });
    }
    else if (result.length === colorPalette_1.CATEGORY_PALETTE.length) {
        // When we hit palette length exactly, last one still uses OTHER_COLOR grey
        // (prototype convention for "Прочее")
        const last = result[result.length - 1];
        last.colorLight = colorPalette_1.OTHER_COLOR.colorLight;
        last.colorDark = colorPalette_1.OTHER_COLOR.colorDark;
        last.colorToken = colorPalette_1.OTHER_COLOR.colorToken;
    }
    return result;
}
function transformProps(chartProps) {
    ...
    const formatValue = v => (0, formatRussian_1.fmtSmart)(v, userDecimals, userSuffix);
    // Axis: suffix-less (shorter labels — "12,4 млрд" instead of "12,4 млрд ₽")
    const formatAxis = v => (0, formatRussian_1.fmtSmart)(v, userDecimals, '');
    const formatPctFn = formatRussian_1.fmtPct;
    ...
    if (mockModeEnabled) {
        const preset = (0, presets_1.getPreset)(formData.mockPreset);
        return {
            width,
            height,
    ...
    const factKey = formData.metricFact ? (0, core_1.getMetricLabel)(formData.metricFact) : null;
    const planKey = formData.metricPlan ? (0, core_1.getMetricLabel)(formData.metricPlan) : null;
    const pyKey = formData.metricPy ? (0, core_1.getMetricLabel)(formData.metricPy) : null;
