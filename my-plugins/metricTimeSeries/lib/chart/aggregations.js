...
exports.toPercentOfPlan = toPercentOfPlan;
 ...
 */
function aggregate(timePoints, categories, gran, selection) {
    ...
    for (const [, group] of groups) {
        ...
        if (gran === 'year') {
            ...
            label = first.monthName || (0, dateHelpers_1.ruMonthName)(first.month);
        }
        else if (gran === 'week') {
            label = `Н${first.week}`;
        }
        ...
        buckets.push({
            label,
            monthShort: (0, dateHelpers_1.ruMonthShort)(first.month),
            monthName: first.monthName || (0, dateHelpers_1.ruMonthName)(first.month),
            year: first.year,
            month: first.month,
            day: first.day,
            week: first.week,
...
function toPercentOfPlan(values, plan) {
