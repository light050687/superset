...
exports.buildXAxis = buildXAxis;
exports.buildYAxis = buildYAxis;
/**
 ...
 */
function buildXAxis(params) {
    const { buckets, gran, tokens, fontText, fontMono } = params;
    const formatter = (_val, idx) => {
        const b = buckets[idx];
        if (!b)
            return '';
    ...
    const axisInterval = gran === 'year' || gran === 'month' ? 0 : 'auto';
    return {
        type: 'category',
...
function buildYAxis(tokens, fontMono, axisFormatter) {
    return {
        type: 'value',
        position: 'left',
        axisLine: { show: false },
        ...
        splitLine: { show: true, lineStyle: { color: tokens.g200, type: [2, 3] } },
        splitNumber: 4,
    };
}
//# sourceMappingURL=buildAxes.js.map