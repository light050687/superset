...
function computeWeekNumber(y, mn) {
...
const SCALE_TO_RUB = 1000000;
function buildWriteoffsDataset() {
    const timePoints = RAW_MONTHS.map((m, i) => {
        const factTotal = CATEGORIES_M.reduce((sum, row) => sum + row[i], 0);
        return {
            t: `${m.y}-${String(m.mn).padStart(2, '0')}-01`,
            year: m.y,
            month: m.mn,
            monthName: (0, dateHelpers_1.ruMonthName)(m.mn),
            day: 1,
    ...
    const categories = CATEGORY_LABELS.map((name, ci) => {
        const values = CATEGORIES_M[ci].map(v => v * SCALE_TO_RUB);
        const total = values.reduce((s, v) => s + (v ?? 0), 0);
        const palette = ci < colorPalette_1.CATEGORY_PALETTE.length ? colorPalette_1.CATEGORY_PALETTE[ci] : colorPalette_1.OTHER_COLOR;
        // Prototype: last category "Прочее" uses grey (--g500)
        const entry = ci === CATEGORY_LABELS.length - 1 ? colorPalette_1.OTHER_COLOR : palette;
        return {
            id: name,
            name,
