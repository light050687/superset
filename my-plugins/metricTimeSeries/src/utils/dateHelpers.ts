...
export function ruMonthName(monthNum: number): string {
  if (monthNum < 1 || monthNum > 12) return '';
  return RU_MONTHS_FULL[monthNum - 1];
}

/** Russian short month name, 1-indexed */
export function ruMonthShort(monthNum: number): string {
  if (monthNum < 1 || monthNum > 12) return '';
 ...
 */
export function parseTimeValue(raw: unknown): Date | null {
