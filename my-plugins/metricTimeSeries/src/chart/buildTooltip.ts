 ...
 */
export function buildTooltipFormatter(params: TooltipBuildParams) {
  ...
  return (rawParams: EChartsTooltipParam | EChartsTooltipParam[]): string => {
    ...
    if (mode === 'line') {
      const byId = new Map<string, EChartsTooltipParam>();
      list.forEach(p => {
        if (p.seriesId) byId.set(p.seriesId, p);
      });
    ...
    reversed.forEach(p => {
      if (!p.seriesId || !p.seriesId.startsWith('cat-')) return;
    ...
    const overlays: Array<{ id: string; lookupKey: string; label: string }> = [
