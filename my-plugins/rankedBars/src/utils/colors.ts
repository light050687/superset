import { CATEGORICAL_TOKENS } from '../themeTokens';
 ...
 */
export function resolveColor(raw: unknown, index: number): string {
  ...
  if (index < CATEGORICAL_TOKENS.length) {
    return `var(${CATEGORICAL_TOKENS[index]})`;
  }
  return 'var(--g500)';
}

/** Extract the raw CSS token (like `--c-sky`) or hex string kept on the row, for animations and a11y. */
export function normalizeColorToken(raw: unknown, index: number): string {
  if (typeof raw === 'string' && raw.length > 0) {
    const trimmed = raw.trim();
    if (HEX_RE.test(trimmed)) {
      return trimmed;
