...
export function getQuadrant(store: StorePoint, t: Thresholds): QuadrantKey {
  const x = store.x;
  const y = store.y;
  if (x >= t.x && y >= t.y) return 'tr';
  if (x < t.x && y >= t.y) return 'tl';
  if (x >= t.x && y < t.y) return 'br';
  return 'bl';
}
...
export function getQuadrantStats(
  stores: StorePoint[],
  t: Thresholds,
): Record<QuadrantKey, QuadrantStats> {
  const stats: Record<QuadrantKey, QuadrantStats> = {
    tl: { count: 0, loss: 0 },
    tr: { count: 0, loss: 0 },
    bl: { count: 0, loss: 0 },
    br: { count: 0, loss: 0 },
 ...
 */
export function storeBadness(s: StorePoint): number {
...
export function getWorstN(stores: StorePoint[], n: number): Set<string> {
