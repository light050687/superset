...
function makeValueFormatter(decimals, unit) {
    ...
    return (n) => {
        ...
        return (0, formatRussian_1.formatRussianSmartEx)(n, decimals, suffix);
    };
}
// ═══════════════════════════════════════
// Main
...
function transformProps(chartProps) {
    ...
    const labelX = metricX ? (0, core_1.getMetricLabel)(metricX) : '';
    const labelY = metricY ? (0, core_1.getMetricLabel)(metricY) : '';
    const labelSize = metricSize ? (0, core_1.getMetricLabel)(metricSize) : '';
    const labelPlanX = metricPlanX ? (0, core_1.getMetricLabel)(metricPlanX) : '';
    const labelPlanY = metricPlanY ? (0, core_1.getMetricLabel)(metricPlanY) : '';
    const labelSumLoss = metricSumLoss ? (0, core_1.getMetricLabel)(metricSumLoss) : '';
    // ── Theme detection ──
    const isDarkMode = (() => {
    ...
    const mock = usingMock ? (0, presets_1.getMockPreset)(String(formData.mockPreset ?? 'retail')) : null;
    // ── Build StorePoint[] ──
    const stores = [];
    if (mock) {
    ...
    const colorScale = core_1.CategoricalColorNamespace.getScale(colorScheme);
    const formatMap = new Map();
    stores.forEach((s) => {
        let meta = formatMap.get(s.format);
        if (!meta) {
            const paletteIndex = formatMap.size % themeTokens_1.DEFAULT_FORMAT_PALETTE.length;
            const tokenKey = themeTokens_1.DEFAULT_FORMAT_PALETTE[paletteIndex];
    ...
    if (metricPlanX || metricPlanY) {
        formatMap.forEach((fmt) => {
            ...
            if (metricPlanX) {
                ...
                fmt.planX = (0, quadrants_1.computeWeightedAverage)(planValues, weights);
            }
            if (metricPlanY) {
                const planValues = sublist.map((s) => s.planY ?? NaN);
                const weights = sublist.map((s) => s.size);
                fmt.planY = (0, quadrants_1.computeWeightedAverage)(planValues, weights);
            }
        });
    }
    ...
    if (thresholdMode === 'static') {
        ...
        thresholdX = (0, quadrants_1.computeAverage)(stores.map((s) => s.x));
        thresholdY = (0, quadrants_1.computeAverage)(stores.map((s) => s.y));
        hasThresholds = stores.length > 0;
    }
    else {
        // 'metric' — средневзвешенное per-row plan
        if (metricPlanX) {
            thresholdX = (0, quadrants_1.computeWeightedAverage)(stores.map((s) => s.planX ?? NaN), stores.map((s) => s.size));
        }
        if (metricPlanY) {
            thresholdY = (0, quadrants_1.computeWeightedAverage)(stores.map((s) => s.planY ?? NaN), stores.map((s) => s.size));
        }
    ...
    const formatCount = (n) => Number.isFinite(n)
        ...
        : '—';
    // ── Labels ──
    const xLabel = formData.xLabel || labelX || 'X';
    const yLabel = formData.yLabel || labelY || 'Y';
