...
exports.getMockPreset = getMockPreset;
...
function generateStores() {
    const rng = (0, scales_1.seededRandom)(20260415);
    const stores = [];
    let id = 0;
    FORMATS.forEach((fmt) => {
        for (let i = 0; i < fmt.count; i++) {
            // X: N(plan, plan*0.35) с heavy-tail outliers
            let x = (0, scales_1.randNormal)(rng, fmt.planX, fmt.planX * 0.35);
            if (rng() < 0.07)
                x = fmt.planX * (1.5 + rng() * 1.5);
            if (rng() < 0.05)
                x = fmt.planX * (0.3 + rng() * 0.4);
            x = Math.max(0.05, x);
            // Y: коррелирована с X слабо
            let y = (0, scales_1.randNormal)(rng, fmt.planY + (x - fmt.planX) * 0.15, fmt.planY * 0.4);
            if (rng() < 0.06)
                y = fmt.planY * (2 + rng() * 4);
            if (rng() < 0.04)
                y = fmt.planY * (0.2 + rng() * 0.4);
            y = Math.max(-0.5, y);
