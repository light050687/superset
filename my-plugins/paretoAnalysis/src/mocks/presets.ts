 ...
 */
export const LOSSES_ITEMS: ParetoItem[] = [
 ...
 */
export const LOSSES_BREAKDOWN: Record<string, BreakdownItem[]> = {
  '190': [
...
export function getBreakdown(id: string, totalRub: number): BreakdownItem[] {
  const explicit = LOSSES_BREAKDOWN[id];
...
export const LOSSES_PRESET: PresetData = {
  items: LOSSES_ITEMS,
...
export const EMPTY_PRESET: PresetData = {
  items: [],
  metricLabel: 'Сумма списаний',
  metricUnit: 'млн ₽',
  metricGenitive: 'всех списаний',
 ...
 */
export function getParetoPreset(
  preset: 'losses' | 'empty' | 'custom' | undefined,
  customJson?: string,
): PresetData {
  if (preset === 'custom' && customJson) {
    try {
      const parsed = JSON.parse(customJson);
      if (!parsed || !Array.isArray(parsed.items)) return EMPTY_PRESET;
      return {
        items: parsed.items as ParetoItem[],
        metricLabel: parsed.metricLabel ?? LOSSES_PRESET.metricLabel,
        metricUnit: parsed.metricUnit ?? LOSSES_PRESET.metricUnit,
        metricGenitive: parsed.metricGenitive ?? LOSSES_PRESET.metricGenitive,
        headerText: parsed.headerText ?? LOSSES_PRESET.headerText,
        breakdownTitle:
          parsed.breakdownTitle ?? LOSSES_PRESET.breakdownTitle,
      };
    } catch {
      return EMPTY_PRESET;
    }
  }
  if (preset === 'empty') return EMPTY_PRESET;
  return LOSSES_PRESET;
}
