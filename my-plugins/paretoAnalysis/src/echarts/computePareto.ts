 ...
 */
function zoneByCumPrev(cumPrev: number, threshold: number): Zone {
  if (cumPrev < threshold) return 'A';
  if (cumPrev < 95) return 'B';
  return 'C';
}
...
export function computePareto(
  ...
  const items = rawItems.map(it => ({
    ...
    valuePrev:
      it.valuePrev != null && Number.isFinite(it.valuePrev)
        ? it.valuePrev
        : null,
    revenueRub:
      it.revenueRub != null && Number.isFinite(it.revenueRub)
        ? it.revenueRub
        : null,
  }));
  ...
  const prevZoneMap: Record<string, Zone> = {};
  if (withPrev.length) {
    const prevSorted = [...withPrev].sort(
      (a, b) => (b.valuePrev as number) - (a.valuePrev as number),
    );
    const prevTotalRaw = prevSorted.reduce(
      (s, i) => s + (i.valuePrev as number),
      0,
    );
    ...
    prevSorted.forEach((it, idx) => {
      ...
      const v = it.valuePrev as number;
  ...
  let cum = 0;
  const result: ComputedParetoItem[] = sorted.map((it, idx) => {
    cum += it.value;
    ...
    const lossPctOfRevenue =
      it.revenueRub != null && it.revenueRub > 0
