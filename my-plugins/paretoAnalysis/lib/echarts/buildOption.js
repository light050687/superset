...
exports.buildEChartsOption = buildEChartsOption;
...
function anchorLabel(it, tokens, showLabels) {
    if (!showLabels)
        return undefined;
    if (it.isNewInA) {
        return {
...
function buildEChartsOption({ computed, state, tokens, }) {
    // Видимые items — учёт Top-A режима.
    const items = state.topAOnly
        ? computed.items.filter(i => i.zone === 'A')
        : computed.items;
    const categories = items.map(i => i.name);
    const totalAll = computed.items.reduce((s, i) => s + i.value, 0) || 1;
    ...
    const barData = items.map(it => {
        ...
        const isDimmed = (state.selectedId != null || state.zoneFilter != null) && !matchesFilter;
        // Значение бара зависит от unit (₽/%).
        const value = state.unit === 'rub' ? it.value : +((it.value / totalAll) * 100).toFixed(2);
        const color = (0, zoneColors_1.zoneColor)(it.zone, tokens);
    ...
    if (state.prevOverlay) {
        series.push({
            ...
            silent: true,
            itemStyle: {
                color: (0, zoneColors_1.toRgba)(tokens.cViolet, 0.35),
                borderRadius: [3, 3, 0, 0],
