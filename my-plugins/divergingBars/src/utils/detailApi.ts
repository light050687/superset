 ...
 */
export function buildStoresPayload(
  params: StoresPayloadParams,
  ...
  const mode: ComparisonMode =
    comparisonMode ?? queryParams.comparisonMode ?? 'prev_period';
  ...
  if (codeCol) columns.push(codeCol);
  if (nameCol && nameCol !== codeCol) columns.push(nameCol);
  if (cityCol) columns.push(cityCol);
  if (formatCol) columns.push(formatCol);
  ...
  // Orderby.
  let orderbyTarget: string | QueryFormMetric;
 ...
 */
export function buildStoresCountPayload(
  params: Omit<
    StoresPayloadParams,
...
export function extractApiRows(
  ...
  const resultArr = json.result as
...
export function extractApiCompRows(
  json: Record<string, unknown>,
