...
export function resolveComparisonRange(
  ...
  currEnd: string,
  mode: ComparisonMode,
  customPrev?: DateRange,
): DateRange | null {
  if (!currStart || !currEnd) return null;
  const ds = dayjs(currStart);
  const de = dayjs(currEnd);
  if (!ds.isValid() || !de.isValid()) return null;
 ...
 */
export function parseSupersetTimeRange(
  timeRange: string | undefined,
): DateRange | null {
  if (!timeRange) return null;
  const trimmed = timeRange.trim();
  if (!trimmed || trimmed === 'No filter') return null;
  ...
  if (!ISO_RE.test(a) || !ISO_RE.test(b)) return null;
 ...
 */
export async function resolveTimeRangeAsync(
  timeRange: string | undefined,
  signal?: AbortSignal,
  ...
  try {
    ...
    const value = (result as { value?: string; error?: string }).value;
...
export function formatRangeDateRu(iso: string): string {
