...
export function fmtRub(v) {
    ...
    if (abs >= 1000000000)
        return `${nf1.format(v / 1000000000)}T`;
    if (abs >= 1000000)
        return `${nf1.format(v / 1000000)}B`;
    if (abs >= 10000)
        return `${nf0.format(v / 1000)}M`;
    if (abs >= 1000)
        return `${nf1.format(v / 1000)}M`;
    return `${nf0.format(v)}к`;
}
/** Полная форма с ₽ после числа (DS 2.0 §11: «1 234 ₽» через неразрывный пробел). */
export function fmtRubWithUnit(v) {
    return `${fmtRub(v)}\u00a0₽`;
}
...
export function fmtSignedPct(pct) {
    ...
    if (pct > 0)
        return `+${nf1.format(abs)}%\u00a0\u2191`;
    if (pct < 0)
        return `\u2212${nf1.format(abs)}%\u00a0\u2193`;
    return `${nf1.format(0)}%`;
}
/** Префикс знака без числа: `+` / `−` (U+2212) / `` (пустая строка). */
export function signPrefix(v) {
    if (v > 0)
        return '+';
    if (v < 0)
        return '\u2212';
 ...
 *  - \u0438\u043d\u0430\u0447\u0435      \u2192 "1,23 \u0442\u0440\u043b\u043d \u20bd"
 */
export function fmtRubFull(v, decimals = 2) {
