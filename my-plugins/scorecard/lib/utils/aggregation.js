...
function computeDelta(current, reference, isPercentMode, deltaFormat, defaultFormatDelta) {
    ...
    if (deltaFormat !== 'auto') {
        ...
        return {
            formatted: (0, formatRussian_1.formatDeltaByFormat)(diff, reference, deltaFormat, isPercentMode),
            status_value: diff,
        };
    }
...
// Row formatting
// ═══════════════════════════════════════
function formatRow(name, metric, comp1, comp2, isPercentMode, formatValue, defaultFormatDelta, colorScheme1, colorScheme2, enableComp1, enableComp2, deltaFormat1, deltaFormat2, fmtComp1, fmtComp2, fmtDelta1, fmtDelta2, showDelta1 = true, showDelta2 = true, delta1Raw = null, delta2Raw = null) {
    const row = {
        name,
