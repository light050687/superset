import type { FormatCode } from '../types';
import { enrichStoreWithMocks, StoreBase } from './storeEnrichment';
 ...
 */
export function generateMockStores(total: 400 | 50 | 0 = 400): ReturnType<
  typeof enrichStoreWithMocks
>[] {
  if (total === 0) return [];
  ...
  const subList: Record<FormatCode, string[]> = {
    express: [
      'Центральная',
      'Северная',
      'Привокзальная',
  ...
  const ratios: Record<FormatCode, number> = {
  ...
  const revenueByFormat: Record<FormatCode, () => number> = {
    express: () => 6 + rng() * 14,
  ...
  const stores: ReturnType<typeof enrichStoreWithMocks>[] = [];
  let idx = 0;
  (Object.keys(FORMATS_META) as FormatCode[]).forEach(formatCode => {
    ...
    for (let i = 0; i < n; i += 1) {
      ...
      if (rng() < 0.07) writeoff = fmt.planWriteoff * (1.5 + rng() * 2.5);
      if (rng() < 0.05) writeoff = fmt.planWriteoff * (0.3 + rng() * 0.4);
      writeoff = Math.max(0.05, writeoff);

      let shrinkage = randNormal(
        rng,
        fmt.planShrinkage + (writeoff - fmt.planWriteoff) * 0.12,
        fmt.planShrinkage * 0.45,
      );
      if (rng() < 0.06) shrinkage = fmt.planShrinkage * (2 + rng() * 4);
      if (rng() < 0.04) shrinkage = fmt.planShrinkage * (0.2 + rng() * 0.4);
...
export function generateByPreset(
  preset: MockPreset,
): ReturnType<typeof enrichStoreWithMocks>[] {
  switch (preset) {
    case 'losses_50':
