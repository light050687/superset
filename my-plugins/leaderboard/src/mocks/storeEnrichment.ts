 ...
 */
export function enrichStoreWithMocks(base: StoreBase): Store {
  ...
  for (let w = 0; w < 12; w += 1) {
    ...
    spark.push(
      +(
        base.writeoff * (0.8 + progress * 0.2) +
        randNormal(rng, 0, base.writeoff * 0.08)
      ).toFixed(2),
    );
  }
  ...
  const causeDist: CauseDistribution[] = (() => {
    const list: CauseDistribution[] = [];
    let remaining = 100;
  ...
  const woTypeDist: WoTypeDistribution[] = (() => {
    const list: WoTypeDistribution[] = [];
  ...
  const segmentsDist: Segment[] = buildSegments(base, rng);
...
function buildSegments(base: StoreBase, rng: () => number): Segment[] {
  ...
  const segments: Segment[] = shuffled.map((segName, j) => {
    const segCode = segName.split(' ')[0];
    const segShortName = segName.replace(`${segCode} `, '');
    const share = weights[j] / weightSum;

    const segWriteoff = +(
      base.writeoff * share * segCount * 0.7 +
      randNormal(rng, 0, base.writeoff * 0.08)
    ).toFixed(2);
    const segShrinkage = +(
      base.shrinkage * share * segCount * 0.7 +
      randNormal(rng, 0, Math.max(0.05, Math.abs(base.shrinkage)) * 0.15)
    ).toFixed(2);
    ...
    const segCauses: CauseDistribution[] = (() => {
      const list: CauseDistribution[] = [];
      let rem = 100;
    ...
    const segWoTypes: WoTypeDistribution[] = (() => {
      const list: WoTypeDistribution[] = [];
      let rem = segWriteoff;
