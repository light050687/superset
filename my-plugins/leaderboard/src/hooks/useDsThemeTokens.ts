...
function isDarkColor(input: string | undefined): boolean {
 ...
 */
export function useDsThemeTokens(): DsThemeBundle {
  const theme = useTheme();
  return useMemo(() => {
    const bg = (theme as { colorBgContainer?: string } | undefined)?.colorBgContainer;
    const dark = isDarkColor(bg);
    const base = dark ? DARK_TOKENS : LIGHT_TOKENS;
    /* Минимально подмешиваем фон/текст из Superset, если заданы */
    const merged: DsTokens = {
      ...base,
      bg: bg ?? base.bg,
      ink:
        (theme as { colorText?: string } | undefined)?.colorText ?? base.ink,
    };
    return {
      tokens: merged,
