...
function TableBodyInner(props) {
    ...
    const globalMaxWriteoff = useMemo(() => Math.max(...allStores.map(s => s.writeoff), 0.01), [allStores]);
    const globalMaxShrinkage = useMemo(() => Math.max(...allStores.map(s => Math.max(0, s.shrinkage)), 0.01), [allStores]);
    if (rows.length === 0) {
        return (_jsx(TableBodyEl, { "$cols": GRID_COLS, children: _jsx("div", { role: "status", "aria-live": "polite", style: {
                    padding: '40px 20px',
                    textAlign: 'center',
                    color: 'var(--g600)',
                    fontFamily: 'var(--m)',
    ...
    return (_jsx(TableBodyEl, { "$cols": GRID_COLS, role: "rowgroup", children: rows.map(r => {
            const data = r.data;
            const id = data.id;
            ...
            return (_jsx(StoreRow, { data: data, level: r.level, displayIdx: r.displayIdx, selected: isCross, dimmed: dimmed, pinned: isPinned, expanded: isExpanded, expandable: expandable, focused: focusedRowId === id, tokens: tokens, globalMaxWriteoff: globalMaxWriteoff, globalMaxShrinkage: globalMaxShrinkage, onRowClick: e => {
