...
exports.generateMockStores = generateMockStores;
exports.generateByPreset = generateByPreset;
const storeEnrichment_1 = require("./storeEnrichment");
 ...
 */
function generateMockStores(total = 400) {
    if (total === 0)
        return [];
    const rng = (0, seededRandom_1.seededRandom)(20260315);
    const cityList = [
        'Хабаровск',
        'Владивосток',
        'Уссурийск',
    ...
    const ratios = {
        express: rankedStoresMock_1.FORMATS_META.express.count,
        minimarket: rankedStoresMock_1.FORMATS_META.minimarket.count,
        super: rankedStoresMock_1.FORMATS_META.super.count,
        home: rankedStoresMock_1.FORMATS_META.home.count,
        superstore: rankedStoresMock_1.FORMATS_META.superstore.count,
    };
    ...
    const stores = [];
    let idx = 0;
    Object.keys(rankedStoresMock_1.FORMATS_META).forEach(formatCode => {
        const fmt = rankedStoresMock_1.FORMATS_META[formatCode];
        const n = counts[formatCode];
        for (let i = 0; i < n; i += 1) {
            let writeoff = (0, seededRandom_1.randNormal)(rng, fmt.planWriteoff, fmt.planWriteoff * 0.35);
            if (rng() < 0.07)
                writeoff = fmt.planWriteoff * (1.5 + rng() * 2.5);
            if (rng() < 0.05)
                writeoff = fmt.planWriteoff * (0.3 + rng() * 0.4);
            writeoff = Math.max(0.05, writeoff);
            let shrinkage = (0, seededRandom_1.randNormal)(rng, fmt.planShrinkage + (writeoff - fmt.planWriteoff) * 0.12, fmt.planShrinkage * 0.45);
            if (rng() < 0.06)
            ...
            const base = {
                ...
                format: formatCode,
                formatName: fmt.name,
                division: rankedStoresMock_1.DIVISION_BY_FORMAT[formatCode],
            ...
            stores.push((0, storeEnrichment_1.enrichStoreWithMocks)(base));
            idx += 1;
        }
    });
    return stores;
}
function generateByPreset(preset) {
    switch (preset) {
        case 'losses_50':
