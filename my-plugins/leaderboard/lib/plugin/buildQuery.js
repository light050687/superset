...
exports.resolvers = exports.BUILD_QUERY_DEFAULTS = void 0;
...
function resolveMetric(fd, camelKey, snakeKey, fallback, legacyTextKey) {
    ...
    if (v) {
        if (typeof v === 'string' && v.trim())
            return v.trim();
        const lbl = (0, core_1.getMetricLabel)(v);
        if (lbl && lbl.trim())
            return lbl.trim();
    }
    if (legacyTextKey) {
...
exports.resolvers = { resolveColumn, resolveMetric };
/**
...
function buildQuery(formData) {
    ...
    return (0, core_1.buildQueryContext)(formData, baseQueryObject => {
        ...
        if (isMockOn) {
            ...
            pushCol(resolveColumn(fd, 'groupbyStoreId', 'groupby_store_id', exports.BUILD_QUERY_DEFAULTS.storeIdCol, { camel: 'storeIdCol', snake: 'store_id_col' }));
            pushCol(resolveColumn(fd, 'groupbyStoreName', 'groupby_store_name', exports.BUILD_QUERY_DEFAULTS.storeNameCol, { camel: 'storeNameCol', snake: 'store_name_col' }));
            pushCol(resolveColumn(fd, 'groupbyCity', 'groupby_city', exports.BUILD_QUERY_DEFAULTS.cityCol, { camel: 'cityCol', snake: 'city_col' }));
            pushCol(resolveColumn(fd, 'groupbyFormat', 'groupby_format', exports.BUILD_QUERY_DEFAULTS.formatCol, { camel: 'formatCol', snake: 'format_col' }));
            pushCol(resolveColumn(fd, 'groupbyFormatName', 'groupby_format_name', exports.BUILD_QUERY_DEFAULTS.formatNameCol, { camel: 'formatNameCol', snake: 'format_name_col' }));
            pushCol(resolveColumn(fd, 'groupbyDivision', 'groupby_division', exports.BUILD_QUERY_DEFAULTS.divisionCol, { camel: 'divisionCol', snake: 'division_col' }));
            pushCol(resolveColumn(fd, 'groupbyToClass', 'groupby_to_class', exports.BUILD_QUERY_DEFAULTS.toClassCol, { camel: 'toClassCol', snake: 'to_class_col' }));
            ...
            const pushMetric = (camel, snake, fallback, legacy) => {
                ...
                const lbl = typeof m === 'string' ? m : (0, core_1.getMetricLabel)(m) ?? '';
                if (lbl && !seenMetricLabels.has(lbl)) {
                    seenMetricLabels.add(lbl);
                    metrics.push(m);
                }
            };
            pushMetric('metricWriteoff', 'metric_writeoff', exports.BUILD_QUERY_DEFAULTS.writeoffMetric, { camel: 'writeoffMetric', snake: 'writeoff_metric' });
            pushMetric('metricShrinkage', 'metric_shrinkage', exports.BUILD_QUERY_DEFAULTS.shrinkageMetric, { camel: 'shrinkageMetric', snake: 'shrinkage_metric' });
            pushMetric('metricPlanWriteoff', 'metric_plan_writeoff', exports.BUILD_QUERY_DEFAULTS.planWriteoffMetric, { camel: 'planWriteoffMetric', snake: 'plan_writeoff_metric' });
            pushMetric('metricPlanShrinkage', 'metric_plan_shrinkage', exports.BUILD_QUERY_DEFAULTS.planShrinkageMetric, { camel: 'planShrinkageMetric', snake: 'plan_shrinkage_metric' });
            pushMetric('metricAvgWriteoff', 'metric_avg_writeoff', exports.BUILD_QUERY_DEFAULTS.avgWriteoffMetric, { camel: 'avgWriteoffMetric', snake: 'avg_writeoff_metric' });
            pushMetric('metricAvgShrinkageCheck', 'metric_avg_shrinkage_check', exports.BUILD_QUERY_DEFAULTS.avgShrinkageCheckMetric, { camel: 'avgShrinkageCheckMetric', snake: 'avg_shrinkage_check_metric' });
        }
