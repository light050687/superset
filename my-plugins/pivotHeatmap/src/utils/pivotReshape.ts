 ...
 */
export function reshapePivot(input: ReshapeInput): ReshapeResult {
  const { data, rowAxisCol, colAxisCol, valueKey, planKey, revenueKey, shopsKey } = input;

  const cells = new Map<string, CellData>();
  const rowSet = new Map<string, AxisItem>();
  const colSet = new Map<string, AxisItem>();
  ...
  data.forEach((row) => {
    ...
    const planPct =
      revenue != null && revenue !== 0 && plan != null ? (plan / revenue) * 100 : null;
    ...
    if (!rowSet.has(rowId)) rowSet.set(rowId, { id: rowId, name: rowId });
    if (!colSet.has(colId)) colSet.set(colId, { id: colId, name: colId });
  });
  ...
  const grandTotal: TotalsSlice | null = cells.size
    ? {
        fact: grandFact,
        ...
        pct:
          grandHasRevenue && grandRevenue !== 0
