...
export const MOCK_COLS: AxisItem[] = [
...
const FORMAT_AVAIL: Record<string, string[]> = {
  express: ['d_express'],
...
export function buildMockCells(): Map<string, MockCellFull> {
  ...
  MOCK_ROWS.forEach((row) => {
    ...
    MOCK_COLS.forEach((col) => {
      if (!avail.includes(col.id)) return;
