import { z } from 'zod';
import { AccentKey, ACCENT_PALETTE, CategoryNode } from '../types';
...
const AccentKeySchema = z.enum(ACCENT_PALETTE as [AccentKey, ...AccentKey[]]);
...
} satisfies Record<string, StructureMockPreset>;
 ...
 */
function parseCustomPreset(json: string | undefined): StructureMockPreset {
 ...
 */
export function getStructurePreset(
  presetKey: string | undefined,
  customJson: string | undefined,
  tokens: Tokens,
): { categories: CategoryNode[]; totalRevenue: number | null } {
  let preset: StructureMockPreset;
  if (presetKey === 'custom') {
  ...
  const categories: CategoryNode[] = preset.categories.map((c) => {
