 ...
 */
export default function buildQuery(formData: StructureDonutFormData) {
  ...
  for (const k of [
    ...
    if (Array.isArray(fd[k])) {
      fd[k] = (fd[k] as unknown[])[0] ?? undefined;
    }
  }

  return buildQueryContext(formData, (baseQueryObject) => {
    const catCol = (fd.groupbyCategory ?? fd.groupby_category) as string | undefined;
    const subCol = (fd.groupbySubcategory ?? fd.groupby_subcategory) as string | undefined;

    const columns: string[] = [];
    if (catCol) columns.push(catCol);
    if (subCol) columns.push(subCol);
    ...
    const query = {
      ...
      orderby:
        allMetrics.length > 0
          ? [[allMetrics[0], false] as [QueryFormMetric, boolean]]
          : [],
      row_limit: 500,
      post_processing: [],
    };
