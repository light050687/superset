...
interface ParentAccumulator {
  name: string;
  rub: number;
  count: number; // накапливаем как number; финально занулим в null если countLabel не задан
...
export function groupRows(
  ...
  const map = new Map<string, ParentAccumulator>();
  ...
  for (const row of rows) {
    ...
    if (isNullish(rawCat)) continue; // строки без категории пропускаем
  ...
  const categories: CategoryNode[] = Array.from(map.values())
