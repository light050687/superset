 ...
 */
export async function fetchDetailRows(params) {
    const { datasourceId, datasourceType, detailGroupby, categoryColumn, categoryValue, metrics, metricLabels, timeRange, granularity, filters, extras, } = params;
    const allFilters = [
    ...
    const json = response
        .json;
    const rawRows = (json?.result?.[0]?.data ?? []);
...
export function safeMetricLabel(m) {
    if (!m)
        return null;
    try {
