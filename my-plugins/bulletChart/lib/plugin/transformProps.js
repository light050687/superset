...
const sparklineBuild_1 = require("../utils/sparklineBuild");
...
function transformProps(chartProps) {
    ...
    if (mockModeEnabled) {
        const preset = (0, presets_1.getPreset)(formData.mockPreset, formData.mockCustomJson, direction, tolerancePct);
        const rows = preset.rows;
        const scaleMax = (0, aggregation_1.computeScaleMax)(rows);
        return {
            width,
            height,
    ...
    const sparkData = sparklineEnabled
        ? (queriesData?.[1]?.data ?? [])
        : [];
    let rows = (0, aggregation_1.buildFormatRows)({
        rawRows: mainData,
    ...
    if (sparklineEnabled && sparkData.length > 0) {
        rows = (0, sparklineBuild_1.attachSparklines)(rows, sparkData, groupbyCategory, formData.metricFact, sparklinePoints);
    }
    const scaleMax = (0, aggregation_1.computeScaleMax)(rows);
    // ── Metric labels для detail API ──
    const factLabel = (0, core_1.getMetricLabel)(formData.metricFact);
    const planLabel = formData.metricPlan ? (0, core_1.getMetricLabel)(formData.metricPlan) : null;
    const pyLabel = formData.metricPy ? (0, core_1.getMetricLabel)(formData.metricPy) : null;
    const storesLabel = formData.metricStores
        ? (0, core_1.getMetricLabel)(formData.metricStores)
        : null;
    ...
    const detailQueryParams = enableDetailModal && detailGroupby
        ...
        : undefined;
    const dataState = !mainData.length || rows.length === 0 ? 'empty' : 'populated';
    return {
        width,
        height,
        dataState,
