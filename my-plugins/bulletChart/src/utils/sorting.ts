import { FormatRow, SortBy } from '../types';

/**
 * Сортировка строк в заданном порядке.
 * Возвращает новый массив — исходный не мутируется.
 */
export function sortRows(rows: FormatRow[], sortBy: SortBy): FormatRow[] {
  const copy = [...rows];
  switch (sortBy) {
    case 'factDesc':
    ...
    case 'deltaPlanDesc':
      return copy.sort(
        (a, b) => (b.deltaPlan ?? -Infinity) - (a.deltaPlan ?? -Infinity),
      );
    case 'deltaPyDesc':
      return copy.sort(
        (a, b) => (b.deltaPy ?? -Infinity) - (a.deltaPy ?? -Infinity),
      );
    case 'storesDesc':
