...
export const IconDd = styled.div<{ open: boolean }>`
  position: absolute;
...
export const FilterPill = styled.button<{ active: boolean; tone: 'bad' | 'good' }>`
  --pill-color: ${({ tone }) => (tone === 'bad' ? 'var(--dn)' : 'var(--up)')};
  --pill-bg-tint: ${({ tone }) =>
    tone === 'bad' ? 'var(--dn-bg)' : 'var(--up-bg)'};
  display: inline-flex;
  align-items: center;
  gap: 6px;
...
export const BRow = styled.div<{ filtered: boolean; dimmed: boolean; statusColor: string }>`
...
export const BBar = styled.div<{ widthPct: number }>`
...
export const BTarget = styled.div<{ leftPct: number }>`
  position: absolute;
  top: -2px;
  bottom: -2px;
...
/* ──────────────────────────────────────────────────────────
   Tooltip
   ────────────────────────────────────────────────────────── */
export const Tooltip = styled.div<{ statusColor: string }>`
  --status-color: ${({ statusColor }) => statusColor};
  position: fixed;
