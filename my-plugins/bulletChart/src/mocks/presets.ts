...
type RawRow = Omit<FormatRow, 'status' | 'deltaPlan' | 'deltaPy'>;

const FORMATS_PROTOTYPE: RawRow[] = [
  {
    id: 'express__0',
    name: 'Экспресс',
    stores: 16,
...
const CATEGORIES_PRESET: RawRow[] = [
...
export function getPreset(
  ...
  const enr = (r: RawRow) => enrichRow(r, direction, tolerancePct);
  ...
  switch (presetName) {
    ...
    case 'long_names':
      return {
        rows: FORMATS_PROTOTYPE.map(r =>
          enr({
            ...r,
            name: `${r.name} с очень длинным названием и уточнениями — филиал №${
    ...
    case 'negative':
      return {
        rows: FORMATS_PROTOTYPE.map((r, i) =>
          enr({
            ...r,
    ...
    case 'custom': {
      if (!customJson) return { rows: [] };
      try {
        const parsed = JSON.parse(customJson) as RawRow[];
